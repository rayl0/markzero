...
import type { Bank, Type } from "@prisma/client";
...
const userSchema = z.object({
    ...
    assignedProductTypes: z.array(z.string().trim().nonempty()).optional()
}).refine((k) => {
    if (k.role === "NORMAL" && (!k.assignedBanks || k.assignedBanks?.length === 0) && (!k.assignedProductTypes || k.assignedBanks?.length === 0)) return false
    return true;
        ...
        try {
            const user = await auth.createUser({
                ...
                attributes: {
                    ...
                    assignedProductTypes: res.data.assignedProductTypes ? res.data.assignedProductTypes as Type[] | undefined : null
                }
            });
