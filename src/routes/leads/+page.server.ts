        ...
        ? await db.dataPoint.findMany({
            where: {
              ...
              OR: [
                ...
                {
                  status: {
                    in: ["Login", "Approved", "UnderwritingOrVerfication"],
                  },
                },
              ],
            },
...
const leadSchema = z.object({
  ...
  type: z.enum(["PL", "BL", "LAP", "HL", "Doctor"]),
...
export const actions = {
  ...
  async addDataPoint({ locals }) {
    ...
    try {
      let isAllowed = false;
      if (
        locals.assignedBanks &&
        locals.assignedBanks.includes(res.data.bank as Bank)
      ) {
        isAllowed = true;
      }
